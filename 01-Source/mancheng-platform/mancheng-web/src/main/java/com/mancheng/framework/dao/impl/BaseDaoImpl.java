package com.mancheng.framework.dao.impl;
...
import com.mancheng.framework.common.log.LogUtil;
import com.mancheng.framework.dao.BaseDao;
import com.mancheng.framework.filter.Filter;
import com.mancheng.framework.filter.Filter.Operator;
import com.mancheng.framework.ordering.Ordering;
import com.mancheng.framework.ordering.Ordering.Direction;
...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
  ...
  private void addRestrictions(CriteriaQuery<T> criteriaQuery, List<Filter> filters) {
    ...
    for (Filter filter : filters) {
      ...
      if (filter.getOperator() == Operator.eq && filter.getValue() != null) {
        ...
        if (filter.getIgnoreCase() != null && filter.getIgnoreCase()
          ...
          if (filter.getValue() instanceof Date) {
              restrictions =
                  criteriaBuilder.and(restrictions, criteriaBuilder.greaterThan(
          ...
          if (filter.getValue() instanceof Date) {
              restrictions =
                  criteriaBuilder.and(restrictions, criteriaBuilder.lessThan(
                      root.<Date>get(filter.getProperty()), (Date) filter.getValue()));
            } else {
              restrictions =
                  criteriaBuilder.and(
      ...
      } else if (filter.getOperator() == Operator.orEqual) {
          restrictions =
  ...
  private String getCountJpql(String jpql) {
    ...
    public List<T> findListCustomized(String jpql, Map<String, ? extends Object> paramMap) {
        if (LogUtil.isDebugEnabled(BaseDaoImpl.class)) {
            LogUtil.debug(BaseDaoImpl.class, "find", "Fetching entities from database with jpql : %s", jpql);
        }
        TypedQuery<T> queryEntity = entityManager.createQuery(jpql, entityClass).setFlushMode(FlushModeType.COMMIT);
        for (final Entry<String, ? extends Object> entry : paramMap.entrySet()) {
            queryEntity.setParameter(entry.getKey(), entry.getValue());
        }
    ...
    public T findSingleResult(String jpql, Map<String, ? extends Object> paramMap) {
        if (LogUtil.isDebugEnabled(BaseDaoImpl.class)) {
            LogUtil.debug(BaseDaoImpl.class, "find", "Fetching entities from database with jpql : %s", jpql);
        }
        TypedQuery<T> queryEntity = entityManager.createQuery(jpql, entityClass).setFlushMode(FlushModeType.COMMIT);
        ...
        return queryEntity.getSingleResult();
    }
